<?php
	...
	class themeZee_SocialMedia_Widget extends WP_Widget {
		function themeZee_SocialMedia_Widget() {
			$widget_ops = array('classname' => 'theme_socialmedia', 'description' => __('Show your Social Media Buttons', 'zeeFlow_language') );
		...
		?>
			...
			<?php 
				...
				if($networks == '') {
					_e('Go to WP-Admin-> Appearance-> Theme Options to configure this widget', 'zeeFlow_language');
				} else {
					echo $networks;
				}
		...
		?>
			<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'zeeFlow_language'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></label></p>
			...
			<label for="<?php echo $this->get_field_id('target'); ?>"><?php _e('Open links in a new window', 'zeeFlow_language'); ?></label></p>
		<?php
		}
	}
