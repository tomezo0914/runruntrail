...
class themezee_Tabbed_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'themezee_tabbed', 'description' => __('Show recent posts with post thumbnails.', 'zeeFlow_language') );
...
<?php
	...
	function widget_content($tab) {
		...
		switch($tab) {
			...
			case 4: // Meta
				...
				$content .= '<li><a href="'.get_bloginfo('rss2_url').'" title="'.esc_attr(__('Syndicate this site using RSS 2.0', 'zeeFlow_language')).'">'. __('Entries <abbr title="Really Simple Syndication">RSS</abbr>', 'zeeFlow_language').'</a></li>';
				$content .= '<li><a href="'.get_bloginfo('comments_rss2_url').'" title="'.esc_attr(__('The latest comments to all posts in RSS', 'zeeFlow_language')).'">'. __('Comments <abbr title="Really Simple Syndication">RSS</abbr>', 'zeeFlow_language').'</a></li>';
				$content .= '<li><a href="'.esc_url('http://wordpress.org/').'" title="'.esc_attr(__('Powered by WordPress, state-of-the-art semantic personal publishing platform.', 'zeeFlow_language')).'">'. __( 'WordPress.org', 'zeeFlow_language') .'</a></li>';
				$content .= wp_meta() . '</ul>';
			break;
			...
			case 7: // Recent Comments
				...
				if ( $comments ) {
					foreach ( (array) $comments as $comment) {
						...
						$content .=  sprintf(_x('%1$s on %2$s', 'widgets', 'zeeFlow_language'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>') . '</li>';
					}
				}
				$content .= '</ul>';
			break;
	...
	function widget_select_options($tab) {
		$options = '<option ' . selected( $tab, 0 ) .' value="0"></option>
					<option ' . selected( $tab, 1 ) .' value="1">'. __('Archives', 'zeeFlow_language') . '</option>
					<option ' . selected( $tab, 2 ) .' value="2">'. __('Categories', 'zeeFlow_language') . '</option>
					<option ' . selected( $tab, 3 ) .' value="3">'. __('Links', 'zeeFlow_language') . '</option>
					<option ' . selected( $tab, 4 ) .' value="4">'. __('Meta', 'zeeFlow_language') . '</option>
					<option ' . selected( $tab, 5 ) .' value="5">'. __('Pages', 'zeeFlow_language') . '</option>
					<option ' . selected( $tab, 6 ) .' value="6">'. __('Popular Posts', 'zeeFlow_language') . '</option>
					<option ' . selected( $tab, 7 ) .' value="7">'. __('Recent Comments', 'zeeFlow_language') . '</option>
					<option ' . selected( $tab, 8 ) .' value="8">'. __('Recent Posts', 'zeeFlow_language') . '</option>
					<option ' . selected( $tab, 9 ) .' value="9">'. __('Tag Cloud', 'zeeFlow_language') . '</option>';
		return $options;
	}
...
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'zeeFlow_language'); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
	
		<div style="background: #eee; padding: 10px; margin-bottom: 10px;">
			<p><label for="<?php echo $this->get_field_id('tab_one_content'); ?>"><strong><?php _e('Tab 1:', 'zeeFlow_language') ?></strong></label>
				...
				<label for="<?php echo $this->get_field_id('tab_one_title'); ?>"><?php _e('Title:', 'zeeFlow_language'); ?></label>
			...
			<p><label for="<?php echo $this->get_field_id('tab_one_content'); ?>"><strong><?php _e('Tab 2:', 'zeeFlow_language') ?></strong></label>
				...
				<label for="<?php echo $this->get_field_id('tab_two_title'); ?>"><?php _e('Title:', 'zeeFlow_language'); ?></label>
			...
			<p><label for="<?php echo $this->get_field_id('tab_one_content'); ?>"><strong><?php _e('Tab 3:', 'zeeFlow_language') ?></strong></label>
				...
				<label for="<?php echo $this->get_field_id('tab_three_title'); ?>"><?php _e('Title:', 'zeeFlow_language'); ?></label>
			...
			<p><label for="<?php echo $this->get_field_id('tab_one_content'); ?>"><strong><?php _e('Tab 4:', 'zeeFlow_language') ?></strong></label>
				...
				<label for="<?php echo $this->get_field_id('tab_four_title'); ?>"><?php _e('Title:', 'zeeFlow_language'); ?></label>
		...
		<strong><?php _e('Settings for Recent/Popular Posts and Recent Comments', 'zeeFlow_language'); ?></strong>
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of entries:', 'zeeFlow_language'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('thumbnails'); ?>"><?php _e('Show Thumbnails?', 'zeeFlow_language'); ?></label></p>
<?php
	}
}
