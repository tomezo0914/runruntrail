...
class themezee_RecentComments_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'themezee_recent_comments', 'description' => __('Show recent comments with gravatar.', 'zeeFlow_language') );
		...
		if ( $comments ) {
			foreach ( (array) $comments as $comment) {
				...
				if ( $post_title == 1 ) :
					...
					$output .= __(' on', 'zeeFlow_language') . ' <a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>';
				else:
					$output .= '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_comment_author_link() . '</a>';

				endif;
				if ( $comment_content == 1 ) :
					$output .= __(' said:', 'zeeFlow_language') . ' <span class="comment-content">' . $this->widget_comment_length($comment->comment_content) . '</span> ';
				endif;
				if ( $comment_date == 1 ) :
...
?>
		...
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of comments to show:', 'zeeFlow_language'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('avatar'); ?>"><?php _e('Show avatar of comment author?', 'zeeFlow_language'); ?></label></p>
		...
		<label for="<?php echo $this->get_field_id('post_title'); ?>"><?php _e('Show post title of commented post?', 'zeeFlow_language'); ?></label></p>
		...
		<label for="<?php echo $this->get_field_id('comment_content'); ?>"><?php _e('Show excerpt of comment?', 'zeeFlow_language'); ?></label></p>
		
		<p><label for="<?php echo $this->get_field_id('comment_length'); ?>"><?php _e('Excerpt length in number of characters:', 'zeeFlow_language'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('comment_date'); ?>"><?php _e('Show date of comment?', 'zeeFlow_language'); ?></label></p>
